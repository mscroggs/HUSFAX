...
class SudokuPage(Page):
    ...
    def calculate(self, calculated):
        ...
        for square in range(9):
            for n in range(1, 10):
                if len([0 for i in range(square % 3, square % 3 + 3)
                        for j in range(square // 3, square // 3 + 3)
                        if n in possible[i][j]]) == 1:
...
class CountdownLettersPage(Page):
    ...
    def background(self):
        ...
        conson = ["B"] * 2 + ["C"] * 3 + ["D"] * 6 + ["F"] * 2 + ["G"] * 3
        conson += ["H"] * 2 + ["J"] * 1 + ["K"] * 1 + ["L"] * 5 + ["M"] * 4
        conson += ["N"] * 8 + ["P"] * 4 + ["Q"] * 1 + ["R"] * 9 + ["S"] * 9
        conson += ["T"] * 9 + ["V"] * 1 + ["W"] * 1 + ["X"] * 1 + ["Y"] * 1
        conson += ["Z"] * 1
    ...
    def generate_content(self):
        ...
        for i, l in enumerate(self.letters):
            ...
            self.add_title(l, font="size4mono", fg="BLUE", bg="BRIGHTWHITE",
                           fill=False, pre=9 * i)
        ...
        for i in range(9, 0, -1):
            if len(self.possible[i]) > 0:
                ...
                for j in range(0, len(self.possible[i]), n):
                    self.add_reveal_text(
                        " ".join(self.possible[i][j:j + n]), wrapping=True)
                    self.add_newline()
                self.add_newline()
...
class CountdownNumbersPage(Page):
        ...
        for i in range(1, 7):
            for operations in product(["+", "-", "*", "/", ";"], repeat=i - 1):
                for comp_order in permutations(range(i - 1)):
                    result, desc = self.calculate(self.numbers[:i],
                                                  operations, comp_order)
    ...
    def calculate(self, numbers, operations, comp_order):
        ...
        while len(operations) > 0:
            ...
            if o == "+":
                result = a + b
                desc += ("    ADD " + cleanstr(a) + " TO"
                         " " + cleanstr(b) + " TO MAKE"
                         " " + cleanstr(result) + ".\n")
            if o == "-":
                result = a - b
                desc += ("    SUBTRACT " + cleanstr(b) + " "
                         "FROM " + cleanstr(a) + " TO MAKE"
                         " " + cleanstr(result) + ".\n")
            if o == "*":
                result = a * b
    ...
    def generate_content(self):
        ...
        self.add_newline()
        self.add_title(" TARGET: " + str(self.target), font="size4",
        ...
        if self.best[2] != self.target:
            self.add_reveal_text("It is impossible to make"
                                 " " + cleanstr(self.target) + ", "
                                 "the best you can make is"
                                 " " + cleanstr(self.best[2]) + ":",
                                 wrapping=True)
            self.add_newline()
...
class GridPuzzlePage(Page):
    ...
    def generate_content(self):
        ...
        self.print_image("wwwww-wwwww-wwwww\n"
                         "wwwww-wwwww-wwwww\n"
                         "ww-ww-ww-ww-ww-ww\n"
                         "ww-ww-ww-ww-ww-ww\n"
                         "wwwww-wwwww-wwwww\n"
        ...
        self.add_wrapped_text("Put the digits 1 to 9 (using each digit "
                              "exactly once) in the boxes so that the "
                              "sums are correct. The sums should be read "
                              "left to right and top to bottom ignoring "
                              "the usual order of operations. For example, "
...
page1 = CrosswordPage(
    ...
    "Y B R\n"
    " MINK",
    ["Winnable animal in Monty Hall problem.", "Animal with horn.",
     "Aerosmith's favourite colour."],
    ["Pokemon rival.",
     "Didn't do the murder because you were doing this instead.",
     "Eat with this, or do it to a GitHub repository."])
...
page3 = CrosswordPage(
    ...
    "151 9\n"
    "95159",
    ["Palindrome.", "Why is 6 afraid of 7?", "5 less than a multiple of 10.",
     "Square.", "EMFFAX page that this puzzle is on.", "Palindrome."],
    ["The sum of this number's digits is 37.", "Multiple of 11.", "Square.",
     "Palindrome.", "Start of made up phone number.", "Factor of 2D."])


page4 = CrosswordPage(
    "154", "Regex crossword", "DUCK\nANON\nTILE\nAXLE",
...
page8 = GridPuzzlePage("158", [2, 5, 9, 4, 3, 7, 8, 6, 1],
                       [90, 84, 48, 64, 90, 63],
                       ["*", "*", "*", "*", "*", "*"],
                       ["*", "*", "*", "*", "*", "*"])
page9 = GridPuzzlePage("159", [5, 3, 9, 7, 1, 6, 8, 4, 2],
                       [17, 1, 0, 4, 12, 27],
                       ["+", "+", "/", "-", "/", "-"],
                       ["+", "-", "/", "*", "*", "/"])
page10 = OllyPage("160")
page11 = PuzzlePage("161", "Ten Digit Number",
                    "Can you create a 10-digit number, where the first "
                    "digit is how many zeros in the number, the second "
                    "digit is how many 1s in the number etc. until the "
                    "tenth digit which is how many 9s in the number?")
page12 = PuzzlePage("162", "The Mutilated Chessboard",
                    "You are given a chessboard where two diagonally "
                    "opposite corners have been removed and a large bag "
                    "of dominoes of such size that they exactly cover "
                    "two adjacent squares on the chessboard.\n\n"
                    "Is it possible to place 31 dominoes on the "
                    "chessboard so that all the squares are covered?")
page13 = PuzzlePage("163", "8! Minutes",
                    "How many days are there in 8! minutes?")
page14 = PuzzlePage("164", "One Two Three", "Each point on a straight "
                    "line is either red or blue. Show that it's always "
                    "possible to find three points of the same color in "
                    "which one is the midpoint of the other two.")
page15 = PuzzlePage("165", "Let The Passenger Train Through!",
                    "A goods train, made up of a locomotive and 5 "
                    "trucks, stops at a small station. The small "
                    "station has a siding which the goods train can "
                    "reverse into. The siding can hold an engine "
                    "and two trucks or three trucks.\n\nA passenger "
                    "train arrives travelling in the opposite "
                    "direction. How can they let it through? (The "
                    "passenger train is too long to fit in the siding.)")
page16 = PuzzlePage("166", "Polya Strikes Out",
                    "Write the numbers 1, 2, 3, ... in a row. "
                    "Strike out every third number beginning "
                    "with the third. Write down the cumulative "
                    "sums of what remains:\n\n"
                    "1, 2, 3, 4, 5, 6, 7, ...\n"
                    "1, 2, X, 4, 5, X, 7, ...\n"
                    "1, 2, 4, 5, 7, ...\n"
                    "1=1; 1+2=3; 1+2+4=7; 1+2+4+5=12; 1+2+4+5+7=19; ...\n"
                    "1, 3, 7, 12, 19, ...\n\n"
                    "Now strike out every second number beginning with"
                    " the second. Write down the cumulative sums of"
                    " what remains. What is the final sequence?")
page17 = PuzzlePage("167", "Combining Multiples",
                    "What is the largest number that cannot "
                    "be written in the form 3a+5b, where a and "
                    "b are (strictly) positive integers?")
page18 = PuzzlePage("168", "Rotating Round Table",
                    "At a large dinner, 24 people are to sit "
                    "evenly spaced around a round table. Place "
                    "cards are laid to show where everyone should "
                    "sit. Unfortunately nobody notices the name "
                    "cards and the guests sit down with nobody in "
                    "the correct seat.\n\nShow that it is possible "
                    "to rotate the table so that at least two "
                    "people will be in the correct seats.")
page19 = PuzzlePage("169", "Reverse Bases",
                    "Find three digits a, b and c such that "
                    "abc in base 10 is equal to cba in base 9?")
