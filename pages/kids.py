...
class NewsroundPage(Page):
    ...
    def background(self):
        ...
        if len(feed) > 1:
            self.entries = [(replace(item["title"]),
                             replace(item["description"]))
                            for item in feed['entries'][:21]]
        else:
            self.entries = []

    def generate_content(self):
        self.add_title("Newsround", bg="MAGENTA", fg="YELLOW",
                       font="size4bold")
        self.add_newline()
...
class BluePeterPage(Page):
    ...
    def generate_content(self):
        ...
        logo = file_handler.load_file("blue-peter")
        logo = logo.replace("-", "w")
        ...
        self.add_text("  " + config.now().strftime("%A").upper() + "      ",
                      fg="YELLOW", bg="RED")
        self.move_cursor(y=5, x=68)
        self.add_text("  " + config.now().strftime("%b %d").upper() + "      ",
                      fg="YELLOW", bg="RED")

        self.move_cursor(x=0, y=8)

        if self.n == 1:
            self.add_title("Badges", font="size4", fg="BLACK",
                           bg="BRIGHTWHITE")
            self.add_wrapped_text("Blue badges", fg="BLUE")
            self.add_wrapped_text(" are awarded for sending interesting "
                                  "letters, stories, makes, pictures, "
                                  "poems, good ideas for the programme, "
                                  "and for having appeared on Blue Peter.")
            self.add_newline()
            self.add_newline()
            self.add_wrapped_text("Green badges", fg="GREEN")
            self.add_wrapped_text(" are awarded for sending in letters, "
                                  "pictures and makes that are about the "
                                  "environment, conservation or nature.")
            self.add_newline()
            self.add_newline()
            self.add_wrapped_text("Silver badges", fg="BRIGHTWHITE")
            self.add_wrapped_text(" are awarded to blue badge holders who "
                                  "go on to make an extra effort.")
            self.add_newline()
            self.add_newline()
            self.add_wrapped_text("Orange badges", fg="ORANGE")
            self.add_wrapped_text(" are given to winners and runners up of "
                                  "Blue Peter competitions.")
            self.add_newline()
            self.add_newline()
            self.add_wrapped_text("Purple badges", fg="MAGENTA")
            self.add_wrapped_text(" are awarded for sending in a review of "
        ...
        if self.n == 2:
            self.add_title("Making Spaghetti", font="size4", fg="BLACK",
                           bg="BRIGHTWHITE")
            self.add_title("           Bolognese", font="size4", fg="BLACK",
                           bg="BRIGHTWHITE")
            ...
            self.add_newline()
            self.add_text("                                 "
                          "CONTINUED on page 134 >>>>>>", fg="YELLOW")
        if self.n == 3:
            ...
            self.add_newline()
            self.add_wrapped_text("2. Begin by chopping your carrots, "
                                  "celery, onions and garlic (always be "
                                  "careful with knives).")
            self.add_newline()
            self.add_wrapped_text("3. Heat up a splash of olive oil into a "
                                  "large pan and add the chopped onion, "
                                  "carrots, celery, garlic and rosemary.")
            self.add_newline()
            self.add_wrapped_text("4. Cook over a medium heat for 10 minutes "
                                  "until soft, turning occassionally.")
            self.add_newline()
            self.add_wrapped_text("5. Add the mince - seasoned with salt and "
                                  "pepper - and cook for a few minutes until "
                                  "browned.")
            self.add_newline()
            self.add_wrapped_text("6. Add the tomato puree, cook for a further"
                                  " 30 seconds and then pour in the chopped "
                                  "tomatoes and stock, bring to the boil and "
                                  "then reduce to a simmer.")
            self.add_newline()
            self.add_wrapped_text("7. Cover loosely with a lid and cook for "
                                  "45 mins - 1 hour, giving the Bolognese a "
                                  "stir every 5-10 minutes.")
            self.add_newline()
            self.add_text("                                 "
                          "CONTINUED on page 135 >>>>>>", fg="YELLOW")
        if self.n == 4:
            self.add_text("CONTINUED from page 134", fg="YELLOW")
            self.add_newline()
            self.add_wrapped_text("8. If the sauce is looking too thick add a "
                                  "little more water, if it’s looking too "
                                  "runny remove the lid and turn the heat "
                                  "down for a few minutes and allow it to "
                                  "thicken.")
            self.add_newline()
            self.add_wrapped_text("9. With 10 minutes to go, cook the pasta "
                                  "according to packet instructions.")
            self.add_newline()
            self.add_wrapped_text("10. Serve the Bolognese over the spaghetti "
                                  "and sprinkle with fresh parsley, a grating "
                                  "of fresh Parmesan and a grind of black "
                                  "pepper.")
            self.add_newline()
            self.add_wrapped_text("TOP TIP: You can also heat up the "
                                  "leftovers in the microwave later if you "
                                  "don't eat it all!", fg="YELLOW")
        if self.n == 5:
            self.add_title("Make a recycled", font="size4", fg="BLACK",
                           bg="BRIGHTWHITE")
            self.add_title("    plastic bag kite", font="size4",
            ...
            self.add_text("METHOD", fg="YELLOW")
            self.add_newline()
            self.add_wrapped_text("1. Take 6 straws and cut off the ends "
                                  "just before the bend.")
            self.add_newline()
            self.add_text("                                 "
                          "CONTINUED on page 137 >>>>>>", fg="YELLOW")
        if self.n == 6:
            self.add_text("CONTINUED from page 136", fg="YELLOW")
            self.add_newline()
            self.add_wrapped_text("2. Feed some string through 5 straws, then "
                                  "cut the string leaving about 10cm at "
                                  "either end.")
            self.add_newline()
            self.add_wrapped_text("3. Arrange the straws into a diamond shape,"
                                  " with one straw going across the middle.")
            self.add_newline()
            self.add_wrapped_text("4. Double knot the string at both sides "
                                  "of the diamond.")
            self.add_newline()
            self.add_wrapped_text("5. Feed some string through a new straw "
                                  "(Leaving about 10cm either side).")
            self.add_newline()
            self.add_wrapped_text("6. Attach the straw to the top of the "
                                  "diamond, then bring the bottom of the "
                                  "straw to the bottom of the diamond. You "
                                  "should be left with a pyramid shape.")
            self.add_newline()
            self.add_wrapped_text("7. Cut along the sides of a plastic bag so "
                                  "that it becomes flat.")
            self.add_newline()
            self.add_wrapped_text("8. Draw around the bottom of the pyramid.")
            self.add_newline()
            self.add_wrapped_text("9. Roll the pyramid onto its side and draw "
                                  "around it again (You should have a diamond "
                                  "shape drawn onto your plastic bag).")
            self.add_newline()
            self.add_wrapped_text("10. Draw 2cm tabs on the four sides of the "
                                  "diamond shape.")
            self.add_newline()
            self.add_wrapped_text("11. Cut out around your shape.")
            self.add_newline()
            self.add_wrapped_text("12. Fold over the tabs and tape to secure "
                                  "the bag to the kite.")
            self.add_newline()
            self.add_wrapped_text("13. Repeat this three times so you have "
                                  "four pyramids.")
            self.add_newline()
            self.add_text("                                 "
                          "CONTINUED on page 138 >>>>>>", fg="YELLOW")
        if self.n == 7:
            self.add_text("CONTINUED from page 137", fg="YELLOW")
            self.add_newline()
            self.add_wrapped_text("14. In order to construct the kite, place "
                                  "3 pyramids so 2 are in front and 1 is "
                                  "behind then one on top(they should look "
                                  "like little tents with open space on the "
                                  "front and bottom!).")
            self.add_newline()
            self.add_wrapped_text("15. Use the leftover string already on "
                                  "your kite to tie the pyramids together.")
            self.add_newline()
            self.add_wrapped_text("16. Gather the ends of the straws (that "
                                  "were originally cut off) and feed them "
                                  "onto a piece of string.")
            self.add_newline()
            self.add_wrapped_text("17. Cut the string once they are all fed "
                                  "through and tie the ends together so you "
                                  "have a long loop.")
            self.add_newline()
            self.add_wrapped_text("18. Then attach this onto the leftover "
                                  "string dangling from the bottom point at "
                                  "the back of the kite.")
            self.add_newline()
            self.add_wrapped_text("19. Feed a long piece of string through "
                                  "the loop on the top of the kite and the "
                                  "middle of the kite (at the back) and tie "
                                  "a knot.")
            self.add_newline()
            self.add_wrapped_text("20. Use this last piece of string to "
                                  "hold on to and get flying!")
        if self.n == 8:
            self.add_title("No-bake biscuits", font="size4", fg="BLACK",
                           bg="BRIGHTWHITE")
            self.add_text("YOU WILL NEED", fg="YELLOW")
            self.add_newline()
            ...
            self.add_text("METHOD", fg="YELLOW")
            self.add_newline()
            self.add_wrapped_text("1. Crush the biscuits, and chop the "
                                  "cherries into small pieces. Grab a "
                                  "grown-up if you need help using a knife! "
                                  "You might find it easier to mix half of "
                                  "the ingredients first, then add the other "
                                  "half.")
            self.add_newline()
            self.add_text("                                 "
                          "CONTINUED on page 140 >>>>>>", fg="YELLOW")
        if self.n == 9:
            self.add_text("CONTINUED from page 139", fg="YELLOW")
            self.add_newline()
            self.add_wrapped_text("TOP TIP: To crush the biscuits, try "
                                  "putting them in a freezer bag and gently "
                                  "bashing them with a mug or rolling pin.",
                                  fg="YELLOW")
            self.add_newline()
            self.add_wrapped_text("2. Mix in the marshmallows and coconut, "
                                  "then pour in the condensed milk and mix "
                                  "thoroughly.")
            self.add_newline()
            self.add_wrapped_text("3. Transfer the mixture into a deep tray. "
                                  "Using your rolling pin, push it flat into "
                                  "the tray so it’s squashed right down.")
            self.add_newline()
            self.add_wrapped_text("4. Leave in the fridge for an hour, then "
                                  "take it out and cut it into slices.")
            self.add_newline()
            self.add_wrapped_text("5. And you're done – share the gooey "
                                  "goodness with your friends and family!")
            self.add_newline()
            self.add_wrapped_text("TOP TIP: You could add a little bit "
                                  "of coconut on top for effect!", fg="YELLOW")
...
class DemonHeadmasterPage(Page):
    ...
    def generate_content(self):
        self.add_title("look into my eyes", font="size4", fg="BLACK",
                       bg="GREEN")
        self.print_image(file_handler.load_file("demon-headmaster"), 4, 0)
        self.add_title("the demon", font="size4", fg="BLACK", bg="GREEN")
        self.add_title("       headmaster", font="size4", fg="BLACK",
                       bg="GREEN")
        self.add_title("Tuesdays 3:10 BBC1", font="size4", fg="BLACK",
                       bg="GREEN")
...
class ChucklePage(Page):
    ...
    def generate_content(self):
        ...
        self.add_title("h", fg="BLACK", bg="GREEN", fill=False, pre=7,
                       font="size4")
        self.move_cursor(y=2)
        self.add_title("u", fg="BLACK", bg="BLUE", fill=False, pre=13,
                       font="size4")
        self.move_cursor(y=2)
        self.add_title("c", fg="BLACK", bg="YELLOW", fill=False, pre=19,
                       font="size4")
        self.move_cursor(y=2)
        self.add_title("k", fg="BLACK", bg="MAGENTA", fill=False, pre=25,
                       font="size4")
        self.move_cursor(y=2)
        self.add_title("l", fg="BLACK", bg="ORANGE", fill=False, pre=31,
                       font="size4")
        self.move_cursor(y=2)
        self.add_title("e", fg="BLACK", bg="GREEN", fill=False, pre=36,
                       font="size4")
        self.move_cursor(y=0)
        self.add_title("V", fg="BLACK", bg="BLUE", fill=False, pre=42)
        self.move_cursor(y=2)
        self.add_title("i", fg="BLACK", bg="YELLOW", fill=False, pre=49,
                       font="size4")
        self.move_cursor(y=2)
        self.add_title("s", fg="BLACK", bg="MAGENTA", fill=False, pre=51,
                       font="size4")
        self.move_cursor(y=2)
        self.add_title("i", fg="BLACK", bg="ORANGE", fill=False, pre=56,
                       font="size4")
        self.move_cursor(y=2)
        self.add_title("o", fg="BLACK", bg="GREEN", fill=False, pre=59,
                       font="size4")
        self.move_cursor(y=2)
        self.add_title("n", fg="BLACK", bg="BLUE", fill=False, pre=64,
                       font="size4")
    ...
    def barry(self):
        ...
        likes = ["Sweets", "Some work", "Gran the Van", "Chocolate eclairs",
                 "Scones with jam and cream", "Banana custard"]
        dislikes = ["Going to the dentist", "Scary stuff and ghosts",
                    "Gorillas", "Heights"]
    ...
    def paul(self):
        ...
        likes = ["Annoying Barry", "Doughnuts & Sweets",
                 "Being the boss or leader", "Inventing",
                 "Relaxing with a cup of tea", "Standing up for himself",
                 "Barry (sometimes)", "Having a rest while Barry works"]
        dislikes = ["When Barry gets what he wants", "Spiders", "Work",
                    "Getting blamed", "Getting told what to do",
                    "Barry's stupid side", "People berating Barry",
                    "Mr No Slacking"]
...
class WackyRacesPage(Page):
    ...
    def generate_content(self):
        ...
        self.add_title("Result", font="size4bold", fg="BLACK",
                       bg="BRIGHTWHITE")
        dnf = randrange(6, 20)
...
class OtisPage(Page):
    ...
    def generate_content(self):
        self.add_title(" Otis's Joke page", fg="BLACK", bg="GREEN",
                       font='size4bold')
        self.print_image(file_handler.load_file("otis"), 5, 0)
        joke = choice([
...
class TorchyPage(Page):
    ...
    def generate_content(self):
        self.add_text(file_handler.load_file("torchy"))
